...
async function testWizardDB() {
  ...
  const { data: proposal, error: pErr } = await supabase.from("proposals").select("*").limit(1).maybeSingle();
  ...
  const { data: budget, error: bErr } = await supabase.from("budget_items").select("*").limit(1).maybeSingle();
  ...
  const { data: lfa, error: lfaErr } = await supabase.from("lfa_rows").select("*").limit(1).maybeSingle();
