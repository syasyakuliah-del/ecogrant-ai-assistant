...
import { AlertCircle, AlertTriangle, CheckCircle2, ClipboardCheck, Eye, FileText, ShieldCheck } from "lucide-react";
import { NARRATIVE_SECTIONS, EXECUTIVE_SUMMARY_KEY, PROPOSAL_STATUSES, STATUS_LABEL } from "@/lib/constants";
...
export function StepReview({ proposal, sections, lfa, budget, donor, save }: StepProps) {
  ...
  const unhandledExceeded = budget.filter((b) => b.validation_status === "exceeded" && !b.override_reason);
  const negativeCosts = budget.filter((b) => Number(b.unit_price) < 0 || Number(b.total) < 0);
  const execSummary = (sections.find((s) => s.section_type === EXECUTIVE_SUMMARY_KEY)?.content ?? "").trim();
  ...
  const isLfaComplete = hasGoal && hasOutcome && hasOutput && hasActivity && hasIndicators && hasTargets;
  ...
  const checklist = [
    {
      ...
      desc: proposal.title ? `Judul, Organisasi (${proposal.organization_name}), Lokasi` : "Judul/Organisasi belum diisi",
    },
    {
      id: 2,
    ...
    },
    {
      ...
      desc: donor?.deadline ? `Tenggat donor: ${formatDate(donor.deadline)}` : "Tenggat tidak dibatasi",
    },
    {
      id: 4,
    ...
    },
    {
      ...
      desc: negativeCosts.length === 0 ? "Seluruh unit price & total RAB bernilai positif" : `Ada ${negativeCosts.length} item biaya negatif`,
    },
    {
      id: 6,
      ok: unhandledExceeded.length === 0,
      title: "6. Item Melebihi SBM / SBU Telah Ditangani",
      desc: unhandledExceeded.length === 0 ? "Seluruh item melampaui standar memiliki justifikasi override" : `Ada ${unhandledExceeded.length} item belum di-override`,
    },
    {
      id: 7,
      ok: isLfaComplete,
      title: "7. Logical Framework Memiliki Goal, Outcome, Output, Activity, Indicator & Target",
      desc: isLfaComplete ? `Matriks LFA lengkap dengan ${lfa.length} baris` : "LFA belum lengkap pada indikator/target",
    },
    {
      id: 8,
      ok: Boolean(execSummary),
      title: "8. Executive Summary (Ringkasan Eksekutif) Tersedia",
      desc: execSummary ? `${execSummary.split(/\s+/).length} kata terisi` : "Executive Summary belum dibuat pada Step 3",
    },
    {
      id: 9,
      ok: true,
    ...
    },
    {
      ...
      desc: missingNarratives.length === 0 ? "Semua 12 bagian narasi terisi" : `${missingNarratives.length} bagian narasi masih kosong`,
    },
  ];
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              <CardTitle className="text-lg flex items-center gap-2">
                <ClipboardCheck className="size-5 text-primary" /> Step 9: Review & Pengujian Kesiapan Proposal
              </CardTitle>
              <CardDescription>
                Hasil peninjauan 11 poin checklist kelayakan dan pratinjau lengkap seluruh bab proposal.
              </CardDescription>
            </div>
            ...
            {checklist.map((item) => (
              <div
                ...
                className={`flex items-start gap-2.5 p-3 rounded-lg border text-xs transition-colors ${
                  item.ok ? "bg-emerald-50/20 border-emerald-500/30" : "bg-amber-50/20 border-amber-500/30"
                }`}
              >
                {item.ok ? (
                ...
                <div>
                  <div className={`font-semibold ${item.ok ? "text-foreground" : "text-amber-800 dark:text-amber-300"}`}>
                    {item.title}
                  </div>
        ...
        <CardContent>
          <Accordion type="single" collapsible className="w-full space-y-2">
            <AccordionItem value="info" className="border rounded-lg px-4">
              <AccordionTrigger className="text-xs font-semibold hover:no-underline">1. Informasi Umum Proposal</AccordionTrigger>
              <AccordionContent className="text-xs space-y-2 pb-4 text-muted-foreground">
                <div><strong>Judul:</strong> {proposal.title}</div>
                <div><strong>Organisasi:</strong> {proposal.organization_name || "-"}</div>
                <div><strong>Lokasi:</strong> {proposal.location}, {proposal.province}</div>
                <div><strong>Nilai Hibah:</strong> {formatCurrency(grant, proposal.currency)}</div>
              </AccordionContent>
            </AccordionItem>
                ...
                {NARRATIVE_SECTIONS.map((sec) => {
                  ...
                  return (
                    <div key={sec.key} className="border-b pb-2">
                      ...
                      <p className="text-muted-foreground mt-1 whitespace-pre-wrap">{content || "(Belum diisi)"}</p>
                    </div>
                  );
                })}
              </AccordionContent>
            ...
            <AccordionItem value="exec" className="border rounded-lg px-4">
              <AccordionTrigger className="text-xs font-semibold hover:no-underline">3. Executive Summary</AccordionTrigger>
              ...
              <AccordionContent className="text-xs pb-4 space-y-1">
                {lfa.map((r, i) => (
                  <div key={r.id} className="p-2 border rounded bg-muted/20">
                    <Badge variant="outline" className="text-[10px] mb-1">{r.row_type.toUpperCase()}</Badge>
                    <div className="font-medium text-foreground">{r.goal || r.outcome || r.output || r.activity}</div>
                    <div className="text-[11px] text-muted-foreground">Indikator: {r.indicator || "-"} | Target: {r.target || "-"}</div>
                  </div>
                ))}
              </AccordionContent>
            </AccordionItem>
            ...
            <AccordionItem value="rab" className="border rounded-lg px-4">
              <AccordionTrigger className="text-xs font-semibold hover:no-underline">
                5. Rencana Anggaran Biaya (Grand Total: {formatCurrency(totals.grandTotal, proposal.currency)})
              </AccordionTrigger>
              <AccordionContent className="text-xs pb-4 space-y-1">
                <div className="font-mono font-bold text-primary">
                  Subtotal: {formatCurrency(totals.subtotal, proposal.currency)} | PPN: {formatCurrency(totals.tax, proposal.currency)} | Grand Total: {formatCurrency(totals.grandTotal, proposal.currency)}
                </div>
                <div className="text-[11px] text-muted-foreground">{budget.length} item biaya terdaftar</div>
              </AccordionContent>
            </AccordionItem>
          </Accordion>
        </CardContent>
