...
export const api = {
  ...
  auth: {
    ...
    logout: async () => {
      ...
      if (data.user)
        await logAudit({ action: "auth.logout", entityType: "auth", entityId: data.user.id });
  ...
  proposals: {
    ...
    submit: async (id: string) => {
      const { data, error } = await supabase
        .from("proposals")
        .update({ status: "siap_ditinjau", submitted_at: new Date().toISOString() })
        .eq("id", id)
        .select()
        .single();
      if (error) throw error;
    ...
    requestRevision: async (id: string, reviewNote: string) => {
      ...
      if (error) throw error;
      await logAudit({
        action: "proposal.request_revision",
        entityType: "proposal",
        entityId: id,
        newValues: { reviewNote },
      });
    ...
    restoreVersion: async (proposalId: string, versionId: string) => {
      ...
      await logAudit({
        action: "proposal.version.restore",
    ...
    members: {
      ...
      invite: async (proposalId: string, userId: string, role: "editor" | "viewer" = "editor") => {
        ...
        if (error) throw error;
        await logAudit({
          action: "proposal.member.invite",
          entityType: "proposal_member",
          entityId: data.id,
  ...
  sectionsAndAI: {
    ...
    generateSection: async (proposalId: string, sectionType: string) => {
      await logAudit({
        action: "ai.generate_section",
        entityType: "proposal_section",
        entityId: proposalId,
        newValues: { sectionType },
      });
      return {
        sectionType,
        generatedContent: `Draft AI untuk seksi ${sectionType} pada proposal...`,
      };
    },
  ...
  logicalFramework: {
    ...
    generate: async (proposalId: string) => {
      await logAudit({
        action: "ai.generate_lfa",
        entityType: "logical_framework",
        entityId: proposalId,
      });
      return { success: true };
    },
  ...
  files: {
    presign: async (filename: string, mimeType: string) => {
      return {
        storageKey: `uploads/${Date.now()}_${filename}`,
        presignedUrl: `https://storage.supabase.co/${filename}`,
      };
    },
    complete: async (
      storageKey: string,
  ...
  admin: {
    getDashboard: async () => {
      const [{ count: userCount }, { count: proposalCount }, { count: donorCount }] =
        await Promise.all([
    ...
    getAnalytics: async () => {
      const { data: proposals } = await supabase
        .from("proposals")
        .select("status, grant_amount, created_at")
        .is("deleted_at", null);
