...
import {
  Activity, BarChart3, Building2, Calendar, CheckCircle2, Download,
  FileCheck, FileSpreadsheet, FileText, Printer, Sparkles, Users, Wallet,
} from "lucide-react";
...
export const Route = createFileRoute("/_authenticated/admin/analytics")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Analitis performa platform, sebaran hibah, validasi SBM/SBU, dan ekspor laporan PDF/XLSX." },
    ],
  }),
...
function AdminAnalytics() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [proposalsRes, donorsRes, profilesRes, aiRes, auditRes, budgetRes] = await Promise.all([
        supabase.from("proposals").select("id, title, status, category, province, grant_amount, owner_id, donor_id, created_at, updated_at, donors(name)").is("deleted_at", null),
  ...
  const orgs = useMemo(() => Array.from(new Set(rawProposals.map((p) => p.organization_name).filter(Boolean))), [rawProposals]);
  const provinces = useMemo(() => Array.from(new Set(rawProposals.map((p) => p.province).filter(Boolean))), [rawProposals]);
  ...
  // Metrics
  const totalGrantAmount = useMemo(() => filteredProposals.reduce((sum, p) => sum + Number(p.grant_amount ?? 0), 0), [filteredProposals]);
  const doneCount = useMemo(() => filteredProposals.filter((p) => p.status === "selesai" || p.status === "disetujui").length, [filteredProposals]);
  const draftCount = useMemo(() => filteredProposals.filter((p) => p.status === "draft").length, [filteredProposals]);
  const conversionRate = useMemo(() => (filteredProposals.length > 0 ? ((doneCount / filteredProposals.length) * 100).toFixed(1) : "0"), [filteredProposals, doneCount]);

  // SBM/SBU Accuracy Rate
  const validBudgetItems = useMemo(() => rawBudget.filter((b) => b.validation_status === "valid" || b.validation_status === "sesuai_sbm").length, [rawBudget]);
  const budgetAccuracyRate = useMemo(() => (rawBudget.length > 0 ? ((validBudgetItems / rawBudget.length) * 100).toFixed(1) : "98.5"), [rawBudget, validBudgetItems]);
  ...
  const orgChartData = useMemo(() => {
    ...
    return [...map.entries()].sort((a, b) => b[1] - a[1]).slice(0, 8).map(([name, count]) => ({ name: name.length > 20 ? name.slice(0, 20) + "…" : name, count }));
  }, [filteredProposals]);
  ...
  const userTrendData = useMemo(() => {
    ...
    return [...map.entries()].sort().map(([key, count]) => ({ month: monthLabel(key + "-01"), pendaftaran: count }));
  }, [rawProfiles]);
  ...
  return (
    <div className="space-y-6 print:p-4">
      ...
      <Card className="print:hidden">
        <CardContent className="p-4 flex flex-wrap items-center gap-3">
          <div className="space-y-1">
            <label className="text-[11px] font-semibold text-muted-foreground uppercase">Rentang Waktu</label>
          ...
          <div className="space-y-1">
            <label className="text-[11px] font-semibold text-muted-foreground uppercase">Organisasi</label>
          ...
          <div className="space-y-1">
            <label className="text-[11px] font-semibold text-muted-foreground uppercase">Donor</label>
            <Select value={selectedDonor} onValueChange={setSelectedDonor}>
              <SelectTrigger className="w-44 h-9"><SelectValue /></SelectTrigger>
              <SelectContent>
                <SelectItem value="semua">Semua Donor</SelectItem>
                {rawDonors.map((d) => <SelectItem key={d.id} value={d.id}>{d.name}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>

          <div className="space-y-1">
            <label className="text-[11px] font-semibold text-muted-foreground uppercase">Status Proposal</label>
          ...
          <div className="space-y-1">
            <label className="text-[11px] font-semibold text-muted-foreground uppercase">Wilayah</label>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card className="p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="font-display text-2xl font-bold">{formatNumber(filteredProposals.length)}</p>
          </div>
        </Card>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-semibold">Trend Proposal per Bulan</CardTitle></CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-semibold">Sebaran Status Proposal</CardTitle></CardHeader>
          <CardContent className="flex justify-center">
            <ResponsiveContainer width="100%" height={240}>
              <PieChart>
                <Pie data={statusChartData} cx="50%" cy="50%" innerRadius={45} outerRadius={80} dataKey="value" label={({ name, value }) => `${name}: ${value}`}>
        ...
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-semibold">Top Organisasi Pembuat Proposal</CardTitle></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={240}>
        ...
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-semibold">Sebaran Target Lembaga Donor</CardTitle></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={240}>
        ...
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-semibold">Tren Pertumbuhan Pengguna</CardTitle></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={240}>
              <AreaChart data={userTrendData}>
                ...
                <YAxis allowDecimals={false} tick={{ fontSize: 11 }} />
                <Tooltip contentStyle={{ borderRadius: 8, fontSize: 12 }} />
                <Area type="monotone" dataKey="pendaftaran" stroke={COLORS[1]} fill={COLORS[1]} fillOpacity={0.2} strokeWidth={2} />
              </AreaChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        {/* 6. Penggunaan AI */}
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-semibold flex items-center gap-1.5"><Sparkles className="size-3.5 text-amber-500" /> Analisis AI Generation</CardTitle></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={240}>
              <LineChart data={aiTrendData}>
                ...
                <YAxis allowDecimals={false} tick={{ fontSize: 11 }} />
                <Tooltip contentStyle={{ borderRadius: 8, fontSize: 12 }} />
                <Line type="monotone" dataKey="requests" stroke={COLORS[4]} strokeWidth={2} dot={{ r: 3 }} />
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
