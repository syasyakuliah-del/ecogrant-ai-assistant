...
import {
  ArrowUpDown, ChevronLeft, ChevronRight, Download, History,
  Pencil, Plus, RotateCcw, Trash2, Upload, ExternalLink,
} from "lucide-react";
...
export const Route = createFileRoute("/_authenticated/admin/donors")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Kelola basis data lembaga donor, prioritas pendanaan, dan kriteria persyaratannya." },
    ],
  }),
...
function AdminDonors() {
  ...
  const { data: auditHistory = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase.from("audit_logs").select("*").eq("entity_id", historyDonorId).order("created_at", { ascending: false }).limit(20);
      return data ?? [];
    },
  });
  ...
  const filtered = useMemo(() => {
    ...
    let result = data.filter((d) => {
      ...
      if (!term) return true;
      return [d.name, d.category, d.country, d.email].some((f) => (f ?? "").toLowerCase().includes(term));
    });
    result.sort((a, b) => {
      const av = a[sortKey] ?? "";
      const bv = b[sortKey] ?? "";
      if (sortKey === "max_grant") return sortDir === "asc" ? Number(av) - Number(bv) : Number(bv) - Number(av);
  ...
  async function handleSave(e: React.FormEvent) {
    e.preventDefault();
    if (!form.name.trim()) { toast.error("Nama donor wajib diisi."); return; }
    ...
    if (error) { toast.error("Gagal menyimpan donor: " + error.message); return; }

    await logAudit({ action: form.id ? "admin.donor.update" : "admin.donor.create", entityType: "donor", entityId: form.id ?? null, newValues: payload });
  ...
  async function softDelete(id: string) {
    ...
    await supabase.from("donors").update({ deleted_at: new Date().toISOString() } as never).eq("id", id);
  ...
  async function restore(id: string) {
    await supabase.from("donors").update({ deleted_at: null } as never).eq("id", id);
  ...
  async function processImport() {
    ...
    if (error) { toast.error("Gagal mengimpor data donor: " + error.message); return; }

    await logAudit({ action: "admin.donor.import", entityType: "donor", newValues: { count: toInsert.length } });
    void qc.invalidateQueries({ queryKey: ["admin-donors-full"] });
  ...
  function exportXLSX() {
    const exportData = (selected.size > 0 ? filtered.filter((d) => selected.has(d.id)) : filtered).map((d) => ({
      "Nama Donor": d.name,
      Kategori: d.category,
      Negara: d.country ?? "-",
      Website: d.website ?? "-",
  ...
  return (
    <div className="space-y-4">
      <PageHeader title="Kelola Donor" description="Basis data lembaga donor, syarat, prioritas pendanaan, dan tenggat waktu." />

      {/* Toolbar */}
      <div className="flex flex-wrap items-center gap-2">
        <div className="flex-1 min-w-56">
          <AdminToolbar query={q} onQueryChange={(v) => { setQ(v); setPage(0); }} placeholder="Cari nama donor, kategori, negara, atau email…" />
        </div>
        ...
        <div className="relative">
          <input type="file" accept=".xlsx,.xls,.csv" onChange={handleFileUpload} className="hidden" id="donor-excel-upload" />
        ...
        <Button onClick={openCreate} className="gap-2 bg-emerald-600 hover:bg-emerald-700 text-white"><Plus className="size-4" /> Tambah Donor</Button>
      </div>
          ...
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="cursor-pointer hover:text-foreground" onClick={() => toggleSort("name")}>
                <span className="flex items-center gap-1">Nama Donor <ArrowUpDown className="size-3 opacity-40" /></span>
              </TableHead>
              ...
              <TableHead className="cursor-pointer hover:text-foreground" onClick={() => toggleSort("max_grant")}>
                <span className="flex items-center gap-1">Max Grant <ArrowUpDown className="size-3 opacity-40" /></span>
              </TableHead>
              <TableHead className="cursor-pointer hover:text-foreground" onClick={() => toggleSort("deadline")}>
                <span className="flex items-center gap-1">Deadline <ArrowUpDown className="size-3 opacity-40" /></span>
              </TableHead>
              ...
              rows.map((d) => (
                <TableRow key={d.id} className={d.deleted_at ? "opacity-50" : ""}>
                  ...
                  <TableCell className="font-medium max-w-56 truncate">
                    <div>{d.name}</div>
                    {d.website && (
                      <a href={d.website} target="_blank" rel="noreferrer" className="text-xs text-emerald-600 hover:underline flex items-center gap-1">
                  ...
                  <TableCell className="text-sm font-mono">{formatCurrency(d.max_grant, d.currency)}</TableCell>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-1">
                      <Button variant="ghost" size="icon" className="size-8" title="Edit donor" onClick={() => openEdit(d)}><Pencil className="size-4" /></Button>
                      <Button variant="ghost" size="icon" className="size-8" title="Riwayat perubahan" onClick={() => setHistoryDonorId(d.id)}><History className="size-4" /></Button>
                      {d.deleted_at ? (
                        <Button variant="ghost" size="icon" className="size-8" title="Restore" onClick={() => void restore(d.id)}><RotateCcw className="size-4 text-blue-600" /></Button>
                      ) : (
                        <Button variant="ghost" size="icon" className="size-8" title="Arsipkan" onClick={() => void softDelete(d.id)}><Trash2 className="size-4 text-red-500" /></Button>
                      )}
                    </div>
                  </TableCell>
                </TableRow>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between text-sm text-muted-foreground">
          <span>Halaman {page + 1} dari {totalPages} ({filtered.length} donor)</span>
      ...
      <Dialog open={formOpen} onOpenChange={setFormOpen}>
        <DialogContent className="sm:max-w-xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{form.id ? "Edit Lembaga Donor" : "Tambah Lembaga Donor Baru"}</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSave} className="space-y-4 py-2">
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-1.5 sm:col-span-2">
                <Label>Nama Donor <span className="text-red-500">*</span></Label>
                <Input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Contoh: Global Environment Facility" />
              </div>
              <div className="space-y-1.5">
                <Label>Kategori</Label>
                <Input value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} placeholder="Yayasan / Multilateral / Government" />
              </div>
              <div className="space-y-1.5">
                <Label>Negara</Label>
                <Input value={form.country} onChange={(e) => setForm({ ...form, country: e.target.value })} placeholder="Indonesia / AS / Jerman" />
              </div>
              <div className="space-y-1.5">
                <Label>Website</Label>
              ...
              <div className="space-y-1.5">
                <Label>Email</Label>
                <Input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="grant@donor.org" />
              </div>
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                ...
                <Input type="number" value={form.min_grant} onChange={(e) => setForm({ ...form, min_grant: e.target.value })} />
              </div>
              <div className="space-y-1.5">
                <Label>Nilai Maksimum Grant</Label>
                <Input type="number" value={form.max_grant} onChange={(e) => setForm({ ...form, max_grant: e.target.value })} />
              </div>
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5 sm:col-span-2">
                ...
                <Textarea rows={2} value={form.funding_fields} onChange={(e) => setForm({ ...form, funding_fields: e.target.value })} placeholder="Konservasi Hutan, Pertanian Berkelanjutan, Restorasi Gambut" />
              </div>
              <div className="space-y-1.5 sm:col-span-2">
                <Label>Prioritas Strategis (pisahkan koma)</Label>
                <Textarea rows={2} value={form.priorities} onChange={(e) => setForm({ ...form, priorities: e.target.value })} placeholder="Masyarakat Adat, Keadilan Gender, Kalimantan" />
              </div>
              <div className="space-y-1.5 sm:col-span-2">
                <Label>Persyaratan Berkas (pisahkan koma)</Label>
                <Textarea rows={2} value={form.requirements} onChange={(e) => setForm({ ...form, requirements: e.target.value })} placeholder="Akta Pendirian, Laporan Keuangan 2 Tahun, Proposal Teknis" />
              </div>
            </div>
            <DialogFooter className="pt-2">
              <Button type="button" variant="outline" onClick={() => setFormOpen(false)}>Batal</Button>
              <Button type="submit" disabled={isSaving} className="bg-emerald-600 hover:bg-emerald-700 text-white">{isSaving ? "Menyimpan…" : "Simpan Donor"}</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={importOpen} onOpenChange={setImportOpen}>
        <DialogContent className="sm:max-w-xl">
          <DialogHeader>
            ...
            <DialogDescription>Periksa sampel data yang akan diimpor dari file Excel.</DialogDescription>
          </DialogHeader>
          <div className="max-h-64 overflow-y-auto rounded border p-2 text-xs space-y-1">
            {importRows.slice(0, 5).map((r, i) => (
              <div key={i} className="p-2 border-b last:border-none flex flex-col gap-0.5">
                <span className="font-semibold text-foreground">{String(r["Nama Donor"] || r["Nama"] || r["name"] || "-")}</span>
                <span className="text-muted-foreground">{String(r["Kategori"] || r["category"] || "-")} · Max: {String(r["Nilai Max"] || r["max_grant"] || "0")}</span>
              </div>
            ))}
            {importRows.length > 5 && <p className="text-center text-muted-foreground py-2">…dan {importRows.length - 5} baris lainnya.</p>}
          </div>
          <DialogFooter>
      ...
      {/* Audit History Modal */}
      <Dialog open={!!historyDonorId} onOpenChange={(o) => { if (!o) setHistoryDonorId(null); }}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><History className="size-5" /> Riwayat Perubahan Data</DialogTitle>
          </DialogHeader>
          <div className="max-h-80 overflow-y-auto space-y-2">
            {auditHistory.length === 0 ? (
              <p className="py-6 text-center text-sm text-muted-foreground">Belum ada riwayat audit tercatat.</p>
            ) : (
              auditHistory.map((h) => (
                <div key={h.id} className="p-3 border rounded-lg text-xs space-y-1">
                  <div className="font-semibold flex justify-between">
                    <span>{h.action}</span>
                    <span className="text-muted-foreground font-normal">{formatDateTime(h.created_at)}</span>
                  </div>
                </div>
              ))
            )}
