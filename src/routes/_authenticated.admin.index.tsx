  ...
  head: () => ({
    meta: [
      { title: "Dashboard Admin — EcoGrant AI" },
      {
        name: "description",
        content:
          "Ringkasan KPI, grafik, pengguna, proposal, donor, dan aktivitas platform EcoGrant AI.",
      },
    ],
  }),
...
function AdminDashboard() {
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [
        ...
        supabase.from("imports").select("id", { count: "exact" }),
        supabase.from("audit_logs").select("id", { count: "exact" }),
        supabase
          .from("proposals")
          .select("id, status, organization_name, donor_id, donors(name), created_at")
          .is("deleted_at", null),
      ...
      const chartProposalMonth = Object.entries(proposalsByMonth).map(([k, v]) => ({
      ...
      const chartProposalStatus = Object.entries(statusCount).map(([k, v]) => ({
      ...
      const chartProposalOrg = Object.entries(orgCount)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 8)
        .map(([k, v]) => ({ organisasi: k.length > 20 ? k.slice(0, 20) + "…" : k, jumlah: v }));
      ...
      const chartProposalDonor = Object.entries(donorCount)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 8)
        .map(([k, v]) => ({ donor: k.length > 20 ? k.slice(0, 20) + "…" : k, jumlah: v }));
      ...
      const chartUserMonth = Object.entries(usersByMonth).map(([k, v]) => ({
        month: monthLabel(k + "-01"),
        pengguna: v,
      }));
      ...
      const chartAI = Object.entries(aiByMonth).map(([k, v]) => ({
        month: monthLabel(k + "-01"),
        permintaan: v.count,
        token: v.tokens,
      }));
  ...
  const kpiCards = [
    ...
    },
    {
      ...
      value: kpi?.proposalsDraft ?? 0,
      icon: FileText,
      to: "/admin/proposals" as const,
      color: "text-gray-500 bg-gray-100 dark:text-gray-400 dark:bg-gray-900",
    },
    {
      label: "Donor Aktif",
      value: kpi?.donorsActive ?? 0,
      icon: Building2,
      to: "/admin/donors" as const,
      color: "text-pink-600 bg-pink-100 dark:text-pink-400 dark:bg-pink-950",
    },
    {
      label: "SBM Aktif",
      value: kpi?.sbmActive ?? 0,
      icon: Coins,
      to: "/admin/sbm" as const,
      color: "text-orange-600 bg-orange-100 dark:text-orange-400 dark:bg-orange-950",
    },
    {
      label: "SBU Aktif",
      value: kpi?.sbuActive ?? 0,
      icon: Wallet,
      to: "/admin/sbu" as const,
    ...
    },
    {
      ...
      value: kpi?.systemActivity ?? 0,
      icon: Activity,
      to: "/admin/audit" as const,
      color: "text-slate-600 bg-slate-100 dark:text-slate-400 dark:bg-slate-900",
    },
  ];
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Ringkasan KPI, grafik, dan aktivitas operasional platform EcoGrant AI."
      />
      ...
      <div className="grid gap-3 grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
        {kpiCards.map((c) => (
          <Link
            key={c.label}
            to={c.to}
            className="surface-panel flex items-center gap-3 p-4 transition-all hover:border-primary/50 hover:shadow-sm group"
          >
            <span
              className={`flex size-10 shrink-0 items-center justify-center rounded-xl ${c.color}`}
            >
              <c.icon className="size-[18px]" />
            </span>
            <div className="min-w-0">
              <p className="text-[11px] text-muted-foreground truncate">{c.label}</p>
              <p className="font-display text-xl font-bold tracking-tight group-hover:text-primary transition-colors">
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-semibold flex items-center gap-1.5">
              <Sparkles className="size-3.5 text-amber-500" /> Penggunaan AI
            </CardTitle>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={220}>
