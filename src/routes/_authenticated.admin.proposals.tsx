...
export const Route = createFileRoute("/_authenticated/admin/proposals")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Tinjau, ubah status, approval, dan kelola seluruh proposal hibah.",
      },
    ],
  }),
...
function AdminProposals() {
  ...
  const filtered = useMemo(() => {
    ...
    const result = data.filter((p) => {
      ...
      if (!term) return true;
      return [p.title, p.organization_name, p.province].some((f) =>
  ...
  async function changeStatus(
    ...
    await logAudit({
      action: "admin.proposal.status",
      entityType: "proposal",
      entityId: id,
      newValues: { status: next, note },
    });
    await notify({
      userId: ownerId,
      type: "status",
      title: `Status proposal: ${STATUS_LABEL[next] ?? next}`,
      message: `Proposal "${title}" kini berstatus ${STATUS_LABEL[next] ?? next}.${note ? ` Catatan: ${note}` : ""}`,
    });
  ...
  function exportXLSX() {
    ...
    void logAudit({
      action: "admin.proposal.export",
      entityType: "proposal",
      newValues: { count: exportData.length, format: "xlsx" },
    });
  }
  ...
  const SortHeader = ({ label, field }: { label: string; field: SortKey }) => (
    ...
    >
      <span className="flex items-center gap-1">
        {label} <ArrowUpDown className="size-3 opacity-40" />
        {sortKey === field && (
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        description="Seluruh proposal dari semua organisasi — approval, arsip, bulk action, dan export."
      />
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="flex-1 min-w-56">
          <AdminToolbar
            ...
            placeholder="Cari judul, organisasi, atau provinsi…"
          />
        </div>
        <Select
          value={statusFilter}
      ...
      {selected.size > 0 && (
        <div className="flex items-center gap-2 rounded-lg border border-primary/30 bg-primary/5 p-3 text-sm">
          ...
          <Button variant="destructive" size="sm" onClick={() => void bulkArchive()}>
            Arsipkan
          </Button>
          <Button variant="outline" size="sm" onClick={exportXLSX}>
            <Download className="size-3.5 mr-1" /> Export Terpilih
          </Button>
                ...
                <TableRow key={p.id} className={p.deleted_at ? "opacity-50" : ""}>
                  ...
                  <TableCell>
                    {p.deleted_at ? (
                      ...
                      <Select
                        value={p.status}
                        onValueChange={(v) => void changeStatus(p.id, p.owner_id, p.title, v)}
                      >
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between text-sm text-muted-foreground">
          <span>
            Halaman {page + 1} dari {totalPages} ({filtered.length} proposal)
          </span>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-2 py-2">
            ...
            <Textarea
              ...
              placeholder={
                approvalTarget?.mode === "approve"
                  ? "Proposal telah memenuhi persyaratan…"
                  : "Mohon perbaiki bagian anggaran dan narasi…"
              }
              rows={3}
            />
          </div>
      ...
      >
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <ClipboardList className="size-5" /> Riwayat Versi Proposal
            </DialogTitle>
          </DialogHeader>
