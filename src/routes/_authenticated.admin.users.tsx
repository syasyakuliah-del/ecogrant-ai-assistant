...
import {
  ChevronLeft, ChevronRight, Clock, Download, KeyRound, Mail, Shield,
...
function AdminUsers() {
  ...
  const { data: loginHistory = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase.from("login_histories").select("*").eq("user_id", loginHistoryUserId).order("created_at", { ascending: false }).limit(20);
      return data ?? [];
    },
  });
  ...
  const allFiltered = (userData ?? []).filter((u) => {
    ...
    if (!term) return true;
    return [u.full_name, u.email, u.organization_name, u.phone].some((f) => (f ?? "").toLowerCase().includes(term));
  });
  ...
  async function handleCreateUser(e: React.FormEvent) {
    e.preventDefault();
    if (!newEmail.trim() || !newPassword.trim()) { toast.error("Email dan kata sandi wajib diisi."); return; }
    if (newPassword.length < 8) { toast.error("Kata sandi minimal 8 karakter."); return; }

    setIsCreating(true);
    const { data: signUpData, error: signUpErr } = await supabase.auth.signUp({
      email: newEmail.trim(),
      password: newPassword,
      options: { data: { full_name: newFullName.trim(), organization_name: newOrg.trim(), phone: newPhone.trim() } },
    });
    setIsCreating(false);

    if (signUpErr) { toast.error("Gagal membuat user: " + signUpErr.message); return; }

    const createdUserId = signUpData.user?.id;
    if (createdUserId && newRole === "admin") {
      const { data: roleRow } = await supabase.from("roles").select("id").eq("name", "admin").maybeSingle();
      await supabase.from("user_roles").upsert({ user_id: createdUserId, role: "admin", role_id: roleRow?.id ?? null } as never, { onConflict: "user_id,role" });
    }
    ...
    setNewFullName(""); setNewEmail(""); setNewPassword(""); setNewPhone(""); setNewOrg(""); setNewRole("user");
  }
  ...
  async function setRole(userId: string, role: string) {
    ...
    if (role === "admin") {
      const { error } = await supabase.from("user_roles").upsert({ user_id: userId, role: "admin", role_id: roleId } as never, { onConflict: "user_id,role" });
      if (error) { toast.error("Peran gagal diubah: " + error.message); return; }
    } else {
  ...
  async function toggleStatus(userId: string, current: string) {
    ...
    if (error) { toast.error("Status gagal diubah."); return; }
  ...
  async function resetPassword(email: string) {
    ...
    if (error) { toast.error("Gagal mengirim tautan reset password: " + error.message); return; }
  ...
  async function resendVerification(email: string) {
    ...
    if (error) { toast.error("Gagal kirim ulang verifikasi: " + error.message); return; }
  ...
  async function togglePermission(roleId: string, permName: string, currentlyAssigned: boolean) {
    ...
    if (!perm) { toast.error("Permission data tidak ditemukan di database."); return; }

    if (currentlyAssigned) {
      const { error } = await supabase.from("role_permissions").delete().eq("role_id", roleId).eq("permission_id", perm.id);
      if (error) { toast.error("Gagal mencabut hak akses: " + error.message); return; }
      toast.success(`Hak akses ${permName} dicabut dari role.`);
    } else {
      const { error } = await supabase.from("role_permissions").insert({ role_id: roleId, permission_id: perm.id });
      if (error) { toast.error("Gagal memberikan hak akses: " + error.message); return; }
  ...
  function exportUsers() {
    ...
    void logAudit({ action: "admin.user.export", entityType: "profile", newValues: { count: exportData.length } });
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Kelola User & RBAC" description="Kelola hak akses, status, peran, riwayat login, dan matriks RBAC." />

      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid w-full grid-cols-2 max-w-md">
          <TabsTrigger value="users" className="gap-2"><Users className="size-4" /> Daftar Pengguna</TabsTrigger>
          <TabsTrigger value="rbac" className="gap-2"><ShieldCheck className="size-4" /> Matriks RBAC</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="users" className="mt-6 space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex-1 min-w-56">
              <AdminToolbar query={q} onQueryChange={(v) => { setQ(v); setPage(0); }} placeholder="Cari nama, email, telepon, atau organisasi…" />
            </div>
            ...
            <Button variant="outline" size="sm" className="gap-1.5" onClick={exportUsers}><Download className="size-3.5" /> Export</Button>
                ...
                {isUsersLoading ? (
                  ...
                  rows.map((u) => (
                    <TableRow key={u.id}>
                      ...
                      <TableCell>
                        <Button variant="ghost" size="sm" onClick={() => void toggleStatus(u.id, u.status)} disabled={u.id === user?.id}>
                          <Badge variant={u.status === "aktif" ? "default" : "secondary"}>{u.status}</Badge>
                        </Button>
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center gap-1">
                          <Button variant="outline" size="sm" className="gap-1 text-[11px] h-7 px-2" onClick={() => void resetPassword(u.email)}>
                            <KeyRound className="size-3" /> Reset
                          </Button>
                          <Button variant="ghost" size="icon" className="size-7" title="Kirim ulang verifikasi" onClick={() => void resendVerification(u.email)}>
                            <Mail className="size-3.5" />
                          </Button>
                          <Button variant="ghost" size="icon" className="size-7" title="Riwayat login" onClick={() => setLoginHistoryUserId(u.id)}>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between text-sm text-muted-foreground">
              <span>Halaman {page + 1} dari {totalPages} ({allFiltered.length} user)</span>
        ...
        <TabsContent value="rbac" className="mt-6 space-y-6">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><Shield className="size-5 text-emerald-600 dark:text-emerald-400" /> Matriks Pemetaan Role & Hak Akses (RBAC)</CardTitle>
              <CardDescription>Tabel di bawah mengelola relasi <code className="text-xs bg-muted px-1.5 py-0.5 rounded">role_permissions</code> dan 21 permission minimum sesuai PRD 5.3.</CardDescription>
            </CardHeader>
            <CardContent>
              {isRbacLoading ? (
                  ...
                  {categories.map((cat) => {
                    ...
                    return (
                      <div key={cat} className="space-y-3">
                        <h4 className="font-semibold text-sm text-emerald-700 dark:text-emerald-300 tracking-wide uppercase">Kategori: {cat}</h4>
                                ...
                                {(rbacData?.roles ?? []).map((r) => {
                                  ...
                                  return (
                                    <label key={r.id} className="flex items-center gap-2 cursor-pointer text-xs">
                                      <Checkbox checked={isAssigned} onCheckedChange={() => void togglePermission(r.id, p.name, isAssigned)} disabled={r.name === "admin" && p.name.startsWith("dashboard")} />
      ...
      <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-emerald-700 dark:text-emerald-400"><UserPlus className="size-5" /> Tambah User & Password Baru</DialogTitle>
            <DialogDescription>Administrator dapat membuatkan akun pengguna operasional atau admin baru secara langsung.</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleCreateUser} className="space-y-4 py-2">
            <div className="space-y-1.5">
              <Label htmlFor="fullname">Nama Lengkap</Label>
              <Input id="fullname" placeholder="Contoh: Budi Santoso" value={newFullName} onChange={(e) => setNewFullName(e.target.value)} />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="email">Alamat Email <span className="text-red-500">*</span></Label>
              <Input id="email" type="email" required placeholder="user@organisasi.or.id" value={newEmail} onChange={(e) => setNewEmail(e.target.value)} />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="password">Kata Sandi (Password) <span className="text-red-500">*</span></Label>
              <Input id="password" type="text" required placeholder="Minimal 8 karakter" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} />
              <div className="text-[11px] text-muted-foreground">Admin dapat menentukan password awal pengguna di sini.</div>
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="phone">Nomor Telepon</Label>
              <Input id="phone" placeholder="08xxxxxxxxxx" value={newPhone} onChange={(e) => setNewPhone(e.target.value)} />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="org">Nama Organisasi / Yayasan</Label>
              <Input id="org" placeholder="Contoh: KSPM Kalimantan Barat" value={newOrg} onChange={(e) => setNewOrg(e.target.value)} />
            </div>
            <div className="space-y-1.5">
            ...
            <DialogFooter className="pt-2">
              ...
              <Button type="submit" disabled={isCreating} className="bg-emerald-600 hover:bg-emerald-700 text-white">{isCreating ? "Membuat Akun…" : "Buat Akun Sekarang"}</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
      ...
      {/* Login History Dialog */}
      <Dialog open={!!loginHistoryUserId} onOpenChange={(o) => { if (!o) setLoginHistoryUserId(null); }}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><Clock className="size-5" /> Riwayat Login</DialogTitle>
          </DialogHeader>
          <div className="max-h-80 overflow-y-auto">
            {loginHistory.length === 0 ? (
              <p className="py-6 text-center text-sm text-muted-foreground">Belum ada riwayat login tercatat.</p>
            ) : (
              <Table>
                <TableHeader><TableRow><TableHead>Waktu</TableHead><TableHead>Status</TableHead><TableHead>IP</TableHead><TableHead>User Agent</TableHead></TableRow></TableHeader>
                <TableBody>
                  {loginHistory.map((h) => (
                    <TableRow key={h.id}>
                      ...
                      <TableCell className="text-xs text-muted-foreground">{h.ip_address ?? "-"}</TableCell>
                      <TableCell className="text-xs text-muted-foreground max-w-40 truncate">{h.user_agent ?? "-"}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
