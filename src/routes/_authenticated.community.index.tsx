...
export const Route = createFileRoute("/_authenticated/community/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Artikel, pengetahuan, dan ruang diskusi antarpengelola program hibah.",
      },
    ],
  }),
...
function CommunityPage() {
  ...
  const filtered = posts.filter((p) => {
    ...
    if (!term) return true;
    return [p.title, p.excerpt, p.category, p.author?.full_name].some((f) =>
  ...
  async function submit() {
    ...
    setBusy(false);
    if (error) {
      toast.error("Artikel gagal disimpan: " + error.message);
      return;
    }
    ...
    toast.success(
      isAdmin
        ...
        : "Artikel tersimpan sebagai Draft dan menunggu moderasi.",
    );
  }
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                ...
                <DialogDescription>
                  Bagikan praktik baik, panduan, atau cerita sukses program organisasi Anda.
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-3 py-2">
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    placeholder="Contoh: Strategi Efektif Menyusun LFA untuk Grant Konservasi"
                  />
                </div>
      ...
      {isLoading ? (
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {filtered.map((post) => (
            <Card
              key={post.id}
              className="flex flex-col justify-between hover:shadow-md transition-shadow overflow-hidden group"
            >
              <div>
                ...
                <div className="p-4 pb-2 flex items-center justify-between gap-2">
                  ...
                  <Badge
                    variant={post.status === "terbit" ? "default" : "outline"}
              ...
              <CardFooter className="pt-2 border-t flex items-center justify-between text-xs text-muted-foreground bg-muted/20">
                <div className="flex items-center gap-2">
                  ...
                  <span className="truncate max-w-28 font-medium text-foreground">
                    {post.author?.full_name || "Penulis"}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  {isAdmin && (
                    <Select
                      value={post.status}
                      onValueChange={(v) => void updateStatus(post.id, v)}
                    >
                      <SelectTrigger className="size-7 p-0 border-none bg-transparent shadow-none">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
