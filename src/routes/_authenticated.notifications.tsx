...
export const Route = createFileRoute("/_authenticated/notifications")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Pusat notifikasi status proposal, ulasan admin, dan pengingat tenggat donor.",
      },
    ],
  }),
...
function NotificationsPage() {
  ...
  const rows = data.filter((n) => {
    ...
    if (filter === "status")
      return ["status", "proposal_approved", "proposal_revision", "proposal_failed"].includes(
        n.type,
      );
    if (filter === "keamanan") return ["security", "account_locked"].includes(n.type);
    if (filter === "data")
      return ["import_success", "import_failed", "export_success", "export_failed"].includes(
        n.type,
      );
    ...
    <div className="mx-auto max-w-4xl space-y-4">
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description="Notifikasi pergerakan status proposal, impor data, dan keamanan akun akan muncul di sini."
        />
      ) : (
