...
export const Route = createFileRoute("/_authenticated/profile")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Kelola identitas, organisasi, jabatan, dan bio pengelola proposal hibah." },
    ],
  }),
...
function ProfilePage() {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    setBusy(false);
    if (error) { toast.error("Profil gagal disimpan: " + error.message); return; }
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <PageHeader title="Profil Pengguna" description="Informasi profil ini digunakan secara otomatis pada header dan lembar pengesahan proposal." />
          ...
          <CardContent className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 border-b pb-6">
              ...
              <div className="flex-1 space-y-1.5 w-full">
                ...
                />
                <p className="text-[11px] text-muted-foreground">Masukkan URL gambar atau gunakan Gravatar.</p>
              </div>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-1.5">
                ...
                <p className="text-[10px] text-muted-foreground">Email utama akun tidak dapat diubah di sini.</p>
              </div>

              <div className="space-y-1.5">
                <Label htmlFor="phone">Nomor Telepon / WhatsApp</Label>
                <Input id="phone" placeholder="08xxxxxxxxxx" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} />
              </div>

              <div className="space-y-1.5">
                <Label htmlFor="position">Jabatan / Peran</Label>
                <Input id="position" placeholder="Direktur Eksekutif / Manager Program" value={form.position} onChange={(e) => setForm({ ...form, position: e.target.value })} />
              </div>

              <div className="space-y-1.5 sm:col-span-2">
                <Label htmlFor="organization_name">Nama Organisasi / Yayasan</Label>
                <Input id="organization_name" placeholder="Contoh: Yayasan Konservasi Borneo" value={form.organization_name} onChange={(e) => setForm({ ...form, organization_name: e.target.value })} />
              </div>

              <div className="space-y-1.5 sm:col-span-2">
                <Label htmlFor="bio">Bio Singkat Pengelola Program</Label>
                <Textarea id="bio" rows={3} placeholder="Tuliskan pengalaman atau latar belakang singkat Anda…" value={form.bio} onChange={(e) => setForm({ ...form, bio: e.target.value })} />
              </div>
            </div>
